...
#the companies below fired all their employees
SELECT * FROM layoffs_staging2
...
#Total_laid_off was more useful than percentage 
SELECT company, AVG(percentage_laid_off) FROM layoffs_staging2
GROUP BY company
ORDER BY 2 DESC; 

SELECT SUBSTRING(`date`,1,7) `month`, SUM(total_laid_off) FROM layoffs_staging2
...
(
SELECT SUBSTRING(`date`,1,7) `month`, SUM(total_laid_off) total_off FROM layoffs_staging2
WHERE SUBSTRING(`date`,1,7) IS NOT NULL
GROUP BY `month`
ORDER BY 1
)
